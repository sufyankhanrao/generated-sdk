...
from shelldigitalpayments.models.mpp_acces_token_response import MppAccesTokenResponse
from shelldigitalpayments.models.prepare_fueling_response import PrepareFuelingResponse
...
class FuelingController(BaseController):
    ...
    def mpp_cancel_fueling(self,
        ...
        return super().new_api_call_builder.request(
            ...
            ResponseHandler()
            .is_api_response(True)
            .local_error('400', 'Error Occurred. The server cannot or will not process the request due to an apparent client error (e.g., malformed request syntax, invalid request message). Please see below for information regarding structure of Response Body vs. all possible errors that could be returned.', CancelFuelingErrorResponseErrorException)
            .local_error('401', 'Unauthorized. Request did not include bearer token or token provided and is invalid.', CancelFuelingErrorResponseErrorException)
            .local_error('403', 'Forbidden. Requestor is not permitted to call the API.', APIException)
            .local_error('404', 'Not Found. Request received by the server but requested URL not found', APIException)
        ).execute()
