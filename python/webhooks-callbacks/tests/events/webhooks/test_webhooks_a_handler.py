...
class TestWebhooksAHandler(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.handler = WebhooksAHandler("dummy_signing_key")
    ...
    def _make_request(body_obj, add_signature_header = True) -> Request:
        raw = TestWebhooksAHandler._json_bytes(body_obj)
        request = Request(
            method="POST",
            path="/webhooks",
        ...
        if add_signature_header:
            request.headers["X-Signature"] = TestWebhooksAHandler._compute_expected_signature(
                secret_key="dummy_signing_key",
                request=request,
            )
        return request

    def test_payment_status_updated_from_webhooks_a_handler(self):
        """
         Tests the `paymentStatusUpdated` event from webhooksAHandler.
        """

        # arrange
        event_payload = {"paymentStatusId": "ps_123"}
        ...
        # assert
        assert isinstance(event, PaymentStatusUpdatedEvent)


    def test_payment_status_created_from_webhooks_a_handler(self):
        """
         Tests the `paymentStatusCreated` event from webhooksAHandler.
        """

        # arrange
        event_payload = {"paymentStatusCreatedId": "ps_123"}
        core_req = self._make_request(event_payload)
        ...
        # assert
        assert isinstance(event, PaymentStatusCreatedEvent)


    def test_signature_verification_failure(self):
        """
         Tests the `SignatureVerificationFailure` case from WebhooksAHandler.
        """
    ...
    def test_unknown_event(self):
        """
         Tests the `UnknownEvent` case from WebhooksAHandler.
        """
