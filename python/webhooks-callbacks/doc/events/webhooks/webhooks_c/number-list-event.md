...
This event's request payload is of type [List[float]].
...
def Webhooks() -> Response:
    ...
    # Step 4: Pattern match for numberListEvent only.
    if isinstance(event, list) and all(isinstance(x, float) for x in event):
