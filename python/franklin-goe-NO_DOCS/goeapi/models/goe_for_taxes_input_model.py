...
class GoeForTaxesInputModel(object):
    ...
    Attributes:
        ...
        is_near_term_volatility (bool): When the near-term volatility
            indicator flashes, this is set to ‘true’. Currently under
            development, <br>
        ...
        curr_date (str): This is an optional parameter that can be used to
            ...
            the specified date. If not passed,  <br>
            the current system date will be used as the current date. Valid
            input format is date – ‘dd-mm-yyyy’
        risk_profile (RiskProfile3): Defines the user’s risk profile. Note
            that Risk Profiles are mapped to portfolio access.  <br>
            For example, a Conservative investor has access to portfolios 1-8
            (or 50% Equity) while an Aggressive  <br>             investor has
            access to all 16 portfolios (up to 90% Equity). This is
            configurable.
        ...
        use_social_security_for_goals (bool): When set to “true” uses Social
            ...
            collected into the taxable accounts.  <br>            If set to
            “true”, a taxable account must be present. This is because there
            can be instances where there are no goals in a given year but an
            incoming social security infusion.  <br>             This infusion
            can only go into a taxable account.
        cashflow_date (str): Cashflow date of the goal - this is the date
            (year is ignored) on which <br>             infusions/withdrawals
            would be realized for the goal. This parameter is optional. <br>
            If not passed or value is null, the algorithm would consider the
            first reallocation date as the cashflow date. <br>
            Scheduled cashflows should be planned for a date prior to the 29th
            of any month.
        ...
        cola_rate (float): Defines the Cost-of-Living Adjustment to be used
            for Social Security amount computations.
        tax_rates (TaxRatesDic2): Consists of tax rates applicable pre &
            post-retirement for the entire household
        household (HouseholdGoeForTaxesObj2): contains all information about
            all the participants.
        accounts (List[GoeToAccountAttr]): Details of different accounts
            associated with unique memberID in the household.  <br>
            Note:  <br>                    A single account can be associated
            with multiple members within the household.  <br>
            This would generally be the case with joint taxable accounts.
        goal_profile_list (List[GoalProfileGoeForTaxesAttr]): Details related
    ...
    def from_dictionary(cls,
        ...
        compute_social_security =\
            ...
            if "computeSocialSecurity" in dictionary.keys() else None
        use_social_security_for_goals =\
            dictionary.get("useSocialSecurityForGoals")\
            if "useSocialSecurityForGoals" in dictionary.keys() else None
        infusion_type =\
            dictionary.get("infusionType")\
            if dictionary.get("infusionType") else None
        cola_rate =\
            dictionary.get("colaRate")\
            if dictionary.get("colaRate") else None
        tax_rates = TaxRatesDic2.from_dictionary(
            dictionary.get("taxRates"))\
            if dictionary.get("taxRates") else None
        household = HouseholdGoeForTaxesObj2.from_dictionary(
        ...
        is_near_term_volatility =\
            ...
            if dictionary.get("isNearTermVolatility") else False
        curr_date =\
        ...
        return cls(is_new_risk_profile,
                   is_new_investment_tenure,
                   is_new_goal_priority,
                   is_new_goal,
                   get_path,
    ...
    def __repr__(self):
        ...
        return (f"{self.__class__.__name__}("
                ...
                f"is_near_term_volatility={(self.is_near_term_volatility
                     if hasattr(self, 'is_near_term_volatility') else None)!r}, "
                ...
                f"use_social_security_for_goals={
                    self.use_social_security_for_goals!r}, "
    ...
    def __str__(self):
        ...
        return (f"{self.__class__.__name__}("
                ...
                f"is_near_term_volatility={(self.is_near_term_volatility
                     if hasattr(self, 'is_near_term_volatility') else None)!s}, "
                ...
                f"use_social_security_for_goals={
                    self.use_social_security_for_goals!s}, "
                f"cashflow_date={(self.cashflow_date
