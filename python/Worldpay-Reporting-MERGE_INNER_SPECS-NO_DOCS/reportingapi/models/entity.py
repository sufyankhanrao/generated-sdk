"""reportingapi.

This file was automatically generated by APIMATIC v3.0 ( https://www.apimatic.io ).
"""

from reportingapi.api_helper import APIHelper


class Entity(object):
    """Implementation of the 'Entity' model.

    Entity details.

    Attributes:
        level (LevelEnum): Support levels such as SalesOrganization/Sales
            Channel (limit 1 value).<br> National, SuperChain, Partner
            (maximum limit of 10 values).<br> Chain, Division, Store and
            Merchant (maximum limit of 2,000 values).
        values (List[str]): Values of hierarchy levels
        chain_code (str): The Chain Code is the level of the merchant
            hierarchy that groups merchant identifiers (MIDs) and any related
            roll-up values under a common identifier for settlement, billing
            and reporting. Chain Code is the primary identifier for merchants
            boarded in MDB (Merchant Database) system. <br> Chain Code is
            mandatory to pass when level is store or division.

    """

    # Create a mapping from Model property names to API property names
    _names = {
        "level": "level",
        "values": "values",
        "chain_code": "chainCode",
    }

    _optionals = [
        "level",
        "values",
        "chain_code",
    ]

    def __init__(self,
                 level=APIHelper.SKIP,
                 values=APIHelper.SKIP,
                 chain_code=APIHelper.SKIP):
        """Initialize a Entity instance."""
        # Initialize members of the class
        if level is not APIHelper.SKIP:
            self.level = level
        if values is not APIHelper.SKIP:
            self.values = values
        if chain_code is not APIHelper.SKIP:
            self.chain_code = chain_code

    @classmethod
    def from_dictionary(cls,
                        dictionary):
        """Create an instance of this model from a dictionary

        Args:
            dictionary (dictionary): A dictionary representation of the object
            as obtained from the deserialization of the server's response. The
            keys MUST match property names in the API description.

        Returns:
            object: An instance of this structure class.

        """
        if not isinstance(dictionary, dict) or dictionary is None:
            return None

        # Extract variables from the dictionary
        level =\
            dictionary.get("level")\
            if dictionary.get("level") else APIHelper.SKIP
        values =\
            dictionary.get("values")\
            if dictionary.get("values") else APIHelper.SKIP
        chain_code =\
            dictionary.get("chainCode")\
            if dictionary.get("chainCode") else APIHelper.SKIP
        # Return an object of this model
        return cls(level,
                   values,
                   chain_code)

    def __repr__(self):
        """Return a unambiguous string representation."""
        return (f"{self.__class__.__name__}("
                f"level={(self.level if hasattr(self, 'level') else None)!r}, "
                f"values={(self.values if hasattr(self, 'values') else None)!r}, "
                f"chain_code={(self.chain_code
                     if hasattr(self, 'chain_code') else None)!r})")

    def __str__(self):
        """Return a human-readable string representation."""
        return (f"{self.__class__.__name__}("
                f"level={(self.level if hasattr(self, 'level') else None)!s}, "
                f"values={(self.values if hasattr(self, 'values') else None)!s}, "
                f"chain_code={(self.chain_code
                     if hasattr(self, 'chain_code') else None)!s})")
