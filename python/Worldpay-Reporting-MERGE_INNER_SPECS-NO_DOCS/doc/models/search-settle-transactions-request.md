...
# Search Settle Transactions Request

## Structure

`SearchSettleTransactionsRequest`
...
| `sort_results_by` | [`SortResultsBy`](../../doc/models/sort-results-by.md) | Optional | A field in a record that is used in determining the final sorted sequence of the records |
| `hierarchy` | [`Entity`](../../doc/models/entity.md) | Required | This object holds the entity and its hierarchy level |
| `date_type` | [`DateType1Enum`](../../doc/models/date-type-1-enum.md) | Optional | Either processDate or transactionDate need to be passed but not both.<br><br>**Constraints**: *Maximum Length*: `16` |
| `date_range` | [`Processdaterange`](../../doc/models/processdaterange.md) | Required | Refers to number of dates that includes a particular start and end date and all dates in between specific periods of time.<br> Date range supported<br><br>07 Days<br>Independent Sales Organization(SO)<br>Independent Sales Affiliate(SA)<br>National(NL)<br>Partner(PT)<br>Super Chain(SC)<br><br>45 days<br>Chain(CH), Merchant(MT). |
| `card_types` | `str` | Optional | Types of card type supported for payments.<br><br>**Constraints**: *Maximum Length*: `6` |
| `card_networks` | [`List[NonrealCardNetworkEnum]`](../../doc/models/nonreal-card-network-enum.md) | Optional | Network which facilitates the payment card transactions as per the card association. |
| `token` | `int` | Optional | Tokenization is a substitution for Primary AccountNumber (PAN) with proxy data. Tokenization limits the ability to conduct fraudulent payment transactions. A token value is numeric and is the same length as the card number. Max length is 19 characters.<br> Either CardNumber or Token is allowed. |
...
| `transaction_amount_range` | [`SearchAuthTransactionsRequesttransactionAmountRange1`](../../doc/models/search-auth-transactions-requesttransaction-amount-range-1.md) | Optional | Used for transaction amount range |
