...
from tester.exceptions.send_boolean_in_model_as_exception import (
    SendBooleanInModelAsException,
)
from tester.exceptions.send_date_in_model_as_exception import (
    SendDateInModelAsException,
)
from tester.exceptions.send_dynamic_in_model_as_exception import (
    SendDynamicInModelAsException,
)
from tester.exceptions.send_long_in_model_as_exception import (
    SendLongInModelAsException,
)
from tester.exceptions.send_number_in_model_as_exception import (
    SendNumberInModelAsException,
)
from tester.exceptions.send_precision_in_model_as_exception import (
    SendPrecisionInModelAsException,
)
from tester.exceptions.send_rfc_1123_in_model_as_exception import (
    SendRfc1123InModelAsException,
)
from tester.exceptions.send_rfc_3339_in_model_as_exception import (
    SendRfc3339InModelAsException,
)
from tester.exceptions.send_string_in_model_as_exception import (
    SendStringInModelAsException,
)
from tester.exceptions.send_unix_time_stamp_in_model_as_exception import (
    SendUnixTimeStampInModelAsException,
)
from tester.exceptions.send_uuid_in_model_as_exception import (
...
class BaseController(object):
    ...
    def global_errors():
        ...
        return {
            ...
            "400": ErrorCase().error_message(
                "400 Global")
                .exception_type(APIException),
            "402": ErrorCase().error_message(
                "402 Global")
                .exception_type(APIException),
            "403": ErrorCase().error_message(
                "403 Global")
                .exception_type(APIException),
            "404": ErrorCase().error_message(
                "404 Global")
                .exception_type(APIException),
            "412": ErrorCase().error_message(
                "Precondition Failed")
                .exception_type(NestedModelException),
            "450": ErrorCase().error_message(
                "caught global exception")
                .exception_type(CustomErrorResponseException),
            "452": ErrorCase().error_message(
                "global exception with string")
                .exception_type(ExceptionWithStringException),
            "453": ErrorCase().error_message(
                "boolean in global exception")
                .exception_type(ExceptionWithBooleanException),
            "454": ErrorCase().error_message(
                "dynamic in global exception")
                .exception_type(ExceptionWithDynamicException),
            "455": ErrorCase().error_message(
                "uuid in global exception")
                .exception_type(ExceptionWithUUIDException),
            "456": ErrorCase().error_message(
                "date in global exception")
                .exception_type(ExceptionWithDateException),
            "457": ErrorCase().error_message(
                "number in global  exception")
                .exception_type(ExceptionWithNumberException),
            "458": ErrorCase().error_message(
                "long in global exception")
                .exception_type(ExceptionWithLongException),
            "459": ErrorCase().error_message(
                "precision in global  exception")
                .exception_type(ExceptionWithPrecisionException),
            "460": ErrorCase().error_message(
                "rfc3339 in global exception")
                .exception_type(ExceptionWithRfc3339DateTimeException),
            "461": ErrorCase().error_message(
                "unix time stamp in global exception")
                .exception_type(UnixTimeStampException),
            "462": ErrorCase().error_message(
                "rfc1123 in global exception")
                .exception_type(Rfc1123Exception),
            "463": ErrorCase().error_message(
                "boolean in model as global exception")
                .exception_type(SendBooleanInModelAsException),
            "464": ErrorCase().error_message(
                "rfc3339 in model as global exception")
                .exception_type(SendRfc3339InModelAsException),
            "465": ErrorCase().error_message(
                "rfc1123 in model as global exception")
                .exception_type(SendRfc1123InModelAsException),
            "466": ErrorCase().error_message(
                "unix time stamp in model as global exception")
                .exception_type(SendUnixTimeStampInModelAsException),
            "467": ErrorCase().error_message(
                "send date in model as global exception")
                .exception_type(SendDateInModelAsException),
            "468": ErrorCase().error_message(
                "send dynamic in model as global exception")
                .exception_type(SendDynamicInModelAsException),
            "469": ErrorCase().error_message(
                "send string in model as global exception")
                .exception_type(SendStringInModelAsException),
            "470": ErrorCase().error_message(
                "send long in model as global exception")
                .exception_type(SendLongInModelAsException),
            "471": ErrorCase().error_message(
                "send number in model as global exception")
                .exception_type(SendNumberInModelAsException),
            "472": ErrorCase().error_message(
                "send precision in model as global exception")
                .exception_type(SendPrecisionInModelAsException),
            "473": ErrorCase().error_message(
                "send uuid in model as global exception")
                .exception_type(SendUuidInModelAsException),
            "500": ErrorCase().error_message(
                "500 Global")
                .exception_type(GlobalTestException),
        }
