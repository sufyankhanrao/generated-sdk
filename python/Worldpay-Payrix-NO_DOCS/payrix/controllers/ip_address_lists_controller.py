...
from payrix.models.iplists_response_result import (
    IplistsResponseResult,
)


class IPAddressListsController(BaseController):
    """A Controller to access Endpoints in the payrix API."""

    def __init__(self, config):
        """Initialize IPAddressListsController object."""
        super(IPAddressListsController, self).__init__(config)

    def get_iplists_id(self,
                       id,
                       request_token=None):
        """Perform a GET request to /iplists/{id}.

        Query an iplist resource that represents a list of IP addresses stored
        as a range between the lowest and highest IP addresses, which can be
        used to whitelist or blacklist particular incoming IP addresses for a
        given Login.

        Args:
            id (str): The ID of this resource and The IP Address List ID.
        ...
        Returns:
            ApiResponse: An object with the response value as well as other
                ...
                response for query on iplists resource retrieves the IP
                Address List.
        ...
        return super().new_api_call_builder.request(
            ...
            .deserialize_into(IplistsResponseResult.from_dictionary)
    ...
    def put_iplists_id(self,
                       ...
                       body,
                       request_token=None):
        """Perform a PUT request to /iplists/{id}.

        _CONFLICT Update an iplist resource that represents a list of IP
        addresses stored as a range between the lowest and highest IP
        addresses, which can be used to whitelist or blacklist particular
        incoming IP addresses for a given Login.

        Args:
            id (str): The ID of this resource and The IP Address List ID.
            body (IplistsPutRequest): Update IP Address List Request
            request_token (str, optional): A custom, one-time identifier for
                any API request (GET, PUT, POST, or DELETE). Blocks future
                requests with the same token for 48 hours, ensuring only the
                first request is processed. Valid values can contain 1-100
        ...
        Returns:
            ApiResponse: An object with the response value as well as other
                ...
                for update on IPLists resource.
    ...
    def delete_iplists_id(self,
                          id,
                          request_token=None):
        """Perform a DELETE request to /iplists/{id}.

        Delete an IP lists resource that represents a list of IP addresses,
        stored as a range between the lowest and highest IP addresses that
        should be included.
        ...
        Returns:
            ApiResponse: An object with the response value as well as other
                useful information such as status codes and headers. Delete IP
                Address List Response for deleting an IPLIST resource.
    ...
    def get_iplists(self,
                    ...
                    page_number=None,
                    page_limit=None):
        """Perform a GET request to /iplists.

        Query an iplist resource representing a list of IP addresses stored as
        a range between the lowest and highest IP addresses that should be
        included, which you can use to whitelist or blacklist particular
        incoming IP addresses for a given Login.
    ...
    def post_iplists(self,
                     body,
                     request_token=None):
        """Perform a POST request to /iplists.

        Create an iplists resource. An IP List represents a list of IP
        (Internet Protocol) addresses, stored as a range between the lowest
        and highest IP addresses that should be included.
        You can use an IP List to whitelist (allow) or blacklist (deny)
        particular incoming IP addresses for a given Login. If you set up a
        whitelist for a Login, then the only allowed IP addresses are those in
        the whitelist, minus any blacklisted IP addresses. If you do not set
        up a whitelist for a Login, then all IP addresses are allowed, except
        for those explicitly blacklisted.
        You can select whether to store IPv4 or IPv6 addresses in an IP list.

        Args:
            body (IplistsPostRequest): Create IP Address List Request
            request_token (str, optional): A custom, one-time identifier for
                any API request (GET, PUT, POST, or DELETE). Blocks future
                requests with the same token for 48 hours, ensuring only the
                first request is processed. Valid values can contain 1-100
        ...
        Returns:
            ApiResponse: An object with the response value as well as other
                useful information such as status codes and headers. Response
                for create on iplists resource
