...
from payrix.models.invoice_results_response_result import (
    InvoiceResultsResponseResult,
)


class InvoiceResultsController(BaseController):
    """A Controller to access Endpoints in the payrix API."""

    def __init__(self, config):
        """Initialize InvoiceResultsController object."""
        super(InvoiceResultsController, self).__init__(config)

    def get_invoice_results_id(self,
                               id,
                               request_token=None):
        """Perform a GET request to /invoiceResults/{id}.

        Query an invoiceResults resource that represents the result of an
        invoice processing, which is generated when a customer pays for an
        invoice.

        Args:
            id (str): The ID of this invoice result.
        ...
        Returns:
            ApiResponse: An object with the response value as well as other
                ...
                for retrieve invoice result response.
        ...
        return super().new_api_call_builder.request(
            ...
            .deserialize_into(InvoiceResultsResponseResult.from_dictionary)
    ...
    def get_invoice_results(self,
                            ...
                            page_number=None,
                            page_limit=None):
        """Perform a GET request to /invoiceResults.

        Query InvoiceResults resource that represents the result of an invoice
        processing. In other words, when the customer pays for an invoice it
        will then generate an invoice result.
        ...
        Returns:
            ApiResponse: An object with the response value as well as other
                useful information such as status codes and headers. Response
                for query on invoiceResults resource
    ...
    def post_invoice_results(self,
                             body,
                             request_token=None):
        """Perform a POST request to /invoiceResults.

        create a invoiceResult.

        Args:
            body (InvoiceResultPostRequest): Create Invoice Result Request
            request_token (str, optional): A custom, one-time identifier for
                any API request (GET, PUT, POST, or DELETE). Blocks future
                requests with the same token for 48 hours, ensuring only the
                first request is processed. Valid values can contain 1-100
        ...
        Returns:
            ApiResponse: An object with the response value as well as other
                useful information such as status codes and headers. Response
                for create on invoiceResults resource
