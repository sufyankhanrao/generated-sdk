...
```python
chargeback_statuses_controller = client.chargeback_statuses
```
...
```python
def get_chargeback_statuses_id(self,
                              id,
                              request_token=None)
```
...
|  --- | --- | --- | --- |
| `id` | `str` | Template, Required | The Chargeback Status ID of this resource. |
| `request_token` | `str` | Header, Optional | A custom, one-time identifier for any API request (GET, PUT, POST, or DELETE). Blocks future requests with the same token for 48 hours, ensuring only the first request is processed. Valid values can contain 1-100 alphanumeric and special characters. See <a href="/apis/payrix/dev-int-guide/references/request-tokens" target="_blank">Request Tokens</a>  for more information. |

## Response Type

This method returns an [`ApiResponse`](../../doc/api-response.md) instance. The `body` property of this instance returns the response data which is of type [`ChargebackStatusesResponseResult`](../../doc/models/chargeback-statuses-response-result.md).
...
result = chargeback_statuses_controller.get_chargeback_statuses_id(
    id,
...
```python
def get_chargeback_statuses(self,
                           request_token=None,
                           search=None,
                           totals=None,
                           page_number=None,
...
result = chargeback_statuses_controller.get_chargeback_statuses(
    request_token=request_token,
    search=search,
    totals=totals,
